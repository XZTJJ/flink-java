package com.zhouhc.streaming.ch06.window.process.evitor;
...
public class TumblingWindowEvitorTemplate {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KeyedStream<Tuple2<String, List<Integer>>, String> tuple2StringKeyedStream = streamSource.map(new MyMap()).keyBy(item -> item.f0);
        //使用剔除器
        DataStream<Tuple2<String, List<Integer>>> hasEvictor = tuple2StringKeyedStream.window(TumblingProcessingTimeWindows.of(Time.seconds(5)))
                .evictor(CountEvictor.of(3)).reduce(new MyReducingFunction());
        //不使用剔除器
        DataStream<Tuple2<String, List<Integer>>> noEvictor = tuple2StringKeyedStream.window(TumblingProcessingTimeWindows.of(Time.seconds(5)))
                .reduce(new MyReducingFunction());
        hasEvictor.print("hasEvictor");
        noEvictor.print("noEvictor");
        environment.execute("TumblingWindowEvitorTemplate");
    }
